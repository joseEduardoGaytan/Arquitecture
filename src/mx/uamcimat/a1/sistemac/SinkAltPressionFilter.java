...
public class SinkAltPressionFilter extends FilterFramework {
	...
	public void run()
    {
		...
		*************************************************************************************/
		...
		double ultimoValorValido = 0;				//Almacena el ultimo valor de presión valido
		...
		**************************************************************/
		...
		try {
			archivo = new FileWriter("ExtremeValuesC1.txt");
			
			archivo.write("Tiempo\t\t\t\tTemperatura(C)\t\tAltitud(M)\t\tPresión(PSI)\r\n"); //Se empieza el encabezado del frame
		...
		try{
			while (true)
			{
				try
				{
					...
					if ( id == 4 )
					{
						 ...
						 */
						if (datos.getPresion() > 90 || datos.getPresion() < 45) {
							list.add(datos);
						}//if
						else{
							if (!list.isEmpty()){
								if (ultimoValorValido==0){
									...
									for (Datos d : list) {
										...
										archivo.write(TimeStampFormat.format(TimeStamp.getTime()) + 
												...
												formatoPresion.format(d.getPresion())+"\r\n");
									}//for
								}//if
								else{
									...
									for (Datos d : list) {
										...
										archivo.write(TimeStampFormat.format(TimeStamp.getTime()) + 
												"\t"+formatoTemperatura.format(d.getTemperatura())+"\t\t"+
												formatoAltitud.format((ultimoValorValido+datos.getAltitud())/2)+"*\t\t"+
												formatoPresion.format(d.getPresion())+"\r\n");
									}//for
							...
							ultimoValorValido = datos.getAltitud();
			...
			System.out.print( "\n" + this.getName() + "::Sink Altitud and Pression Filter Exiting; bytes read: " + bytesread +"; "  );
		}
